...
use PhpWord;
class BeforeController extends CommonController {
    public function pp(){
    ...
    public function index(){
        ...
        if($belong_year==''){
          $belong_year=date('Y',time())-1;
        }
        $belong_year_1=date('Y',time())-1;
    ...
    public function pay_show(){
      ...
      $this->assign('time_list',$ss);//时间
      $this->assign('count_num',$count_num);//总编条数
      $this->assign('count_year',$belong_year_1);//当前年份
...
public function contract_add(){  //后面看是否
    ...
    public function list_edit(){
      ...
      public function tt(){
        ...
        $pay_amount='1000';
         $contract=M('contract');
        $map_2['id']=$contract_id;
        $ss=$contract->where($map_2)->field('contract_amount,contract_exact_amount')->find();
        if($ss['contract_exact_amount']==null){
          $ratio=round(($pay_amount/$ss['contract_amount']),4)*'100'."%";
        }else{
           $ratio=round(($pay_amount/$ss['contract_exact_amount']),4)*'100'."%";
        }
        dump($ratio);
