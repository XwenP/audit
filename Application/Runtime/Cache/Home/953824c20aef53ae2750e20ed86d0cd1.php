    ...
    <aside class="Hui-aside">
        <div class="menu_dropdown bk_2">
            <dl>
                ...
                <dd>
                    <ul>
                        <li><a href="/audit/index.php/Home/Index/index" title="<?php echo ($now_year); ?>年合同总表"><?php echo ($now_year); ?>年工程项目合同付款登记表</a></li>
                    </ul>
                </dd>
            </dl>
            <dl>
                ...
                <dd style="display: block;">
                    <ul>
                        <?php if(is_array($item_list)): foreach($item_list as $key=>$v): if($v["item_id"] == $data3['item_id']): ?><li class="current"><a href="/audit/index.php/Home/Index/list_show?item_id=<?php echo ($v["item_id"]); ?>" title="<?php echo ($v["item_name"]); ?>"><?php echo ($v["item_name"]); ?></a></li>
                                <?php else: ?>
                                <li><a href="/audit/index.php/Home/Index/list_show?item_id=<?php echo ($v["item_id"]); ?>" title="<?php echo ($v["item_name"]); ?>"><?php echo ($v["item_name"]); ?></a></li><?php endif; endforeach; endif; ?>
                    </ul>
                </dd>
            </dl>
            <dl>
        ...
        <div class="Hui-article">
            <article class="cl pd-20">
                <div class="cl pd-5 bg-1 bk-gray mt-10">
                    <span class="l">
                        <a href="/audit/index.php/Home/Index/item_out?item_id=<?php echo ($data3["item_id"]); ?>" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe644;</i> 导出分表</a>
                        <a href="javascript:;" item_id="<?php echo ($data3["item_id"]); ?>" class="btn_end btn btn-success radius"><i class="Hui-iconfont">&#xe615;</i> 结束项目</a>
                    </span>
                    <span class="r">共有数据：<strong><?php echo count($data1);?></strong> 条</span>
                </div>
                <div style="float: right; margin:10px 0px;"><span>按项目名称检索:</span><input class="ser" type="text" style="padding: 0px 10px;margin-left:15px; height: 30px; width: 180px; border: 1px solid #eee;"/></div>
                <div class="mt-20">
                    <table class="table table-border table-bordered table-hover1 table-bg table-sort">
                        <thead>
                            <tr>
                                <th colspan="14">
                                    ...
                                    <h5><strong>已签订各项合同金额合计：<?php echo (number_format($data3["contract_amount_all"],2)); ?></strong></h5>
                                    <h5><strong>项目已付款项金额合计 ：<?php echo (number_format($data3["add_all_pay"],2)); ?></strong></h5>                          </th>
                            </tr>
                            <tr class="text-c">
                                <th width="50">序号</th>
                        ...
                        <?php if(is_array($data1)): foreach($data1 as $key=>$v): if($v["contract_nature"] == '招标代理' ): ?><tbody>
                                    <?php if(!empty($v['paylist'][0])): ?><tr>
                                            ...
                                            <td class="edit" rowspan="<?php echo count($v['paylist'])*2+3;?>"><?php echo (number_format($v["contract_amount"],2)); ?></td>
                                            <td class="edit" rowspan="<?php echo count($v['paylist'])*2+3;?>"><?php echo (number_format($v["contract_exact_amount"],2)); ?></td>
                                            ...
                                            <?php if(!empty($v["total_invest"])): ?><td class="edit1" width="50"><?php echo (number_format($v["total_invest"],2)); ?></td>
                                                <?php else: ?>
                                                <td class="edit1" width="50">-</td><?php endif; ?>
                                            <td class="edit1" width="50"><b>建安费</b></td>
                                            <?php if(!empty($v["jian_fee"])): ?><td class="edit1" width="50"><?php echo (number_format($v["jian_fee"],2)); ?></td>
                                                <?php else: ?>
                                                <td class="edit1" width="50">-</td><?php endif; ?>
                                        </tr>
                                        <?php else: ?>
                                        <tr>
                                            ...
                                            <td class="edit" rowspan="5"><?php echo (number_format($v["contract_amount"],2)); ?></td>
                                            <td class="edit" rowspan="5"><?php echo (number_format($v["contract_exact_amount"],2)); ?></td>
                                            <td width="70"><b>预付时间1</b></td>
                                            <td width="80"><b>预付金额1</b></td>
                                            <td width="60"><b>预付比例1</b></td>
                                    ...
                                    <tr>
                                        <?php if(!empty($v['paylist'][0])): ?><td class="edit"><?php echo ($v['paylist'][0]['pay_time']); ?></td>
                                            <td class="edit"><?php echo (number_format($v['paylist'][0]['pay_amount'],2)); ?></td>
                                        ...
                                        <?php if(!empty($v["budget_amount"])): ?><td class="edit1"><?php echo (number_format($v["budget_amount"],2)); ?></td>
                                            <?php else: ?>
                                            <td class="edit1">-</td><?php endif; ?>
                                        <td class="edit1"><b>控制价</b></td>
                                        <?php if(!empty($v["limit_price"])): ?><td class="edit1"><?php echo (number_format($v["limit_price"],2)); ?></td>
                                            <?php else: ?>
                                            <td class="edit1">-</td><?php endif; ?>
                                    </tr>
                                    <?php if(is_array($v['paylist'])): $i = 0; $__LIST__ = array_slice($v['paylist'],1,null,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <tr>
                                            <?php if(!empty($vo)): ?><td class="edit"><?php echo ($vo["pay_time"]); ?></td>
                                                <td class="edit"><?php echo (number_format($vo["pay_amount"],2)); ?></td>
                                    ...
                                    <tr>
                                        <td><b>累计预付</b></td>
                                        <?php if(!empty($v["contract_add_money"])): ?><td class="edit" colspan="2"><?php echo (number_format($v["contract_add_money"],2)); ?></td>
                                            <?php else: ?>
                                    ...
                                    <tr>
                                        <td><b>余额</b></td>
                                        <?php if(!empty($v["contract_balance"])): ?><td class="edit" colspan="2"><?php echo (number_format($v["contract_balance"],2)); ?></td>
                                            <?php else: ?>
                                            <td class="edit" colspan="2">-</td><?php endif; ?>
                                    </tr>
                                    <tr>
...
<script type="text/javascript" src="/audit/Public/lib/laypage/1.2/laypage.js"></script>
    ...
    $(".edit1").click(function() {
        note();
        var contract_id = $(this).parent().parent().find('.co_id').text();
        var string = '/audit/index.php/Home/Index/extra_edit' + '?contract_id=' + contract_id;
        list_edit('招标费用情况', string, '', '420');
    });
    $(".edit2").click(function() {
        note();
        var contract_id = $(this).parent().parent().find('.co_id').text();
        var string = '/audit/index.php/Home/Index/nature_edit' + '?contract_id=' + contract_id;
        list_edit('合同性质', string, '400', '200');
    });
    $(".edit3").click(function() {
        note();
        var contract_id = $(this).parent().parent().find('.co_id').text();
        var string = '/audit/index.php/Home/Index/unit_edit' + '?contract_id=' + contract_id;
        list_edit('承包单位', string, '400', '200');
    });
    $(".edit4").click(function() {
        note();
        var contract_id = $(this).parent().parent().find('.co_id').text();
        var string = '/audit/index.php/Home/Index/name_edit' + '?contract_id=' + contract_id;
        list_edit('项目名称', string, '600', '200');
    });
    $(".remarks").click(function() {
        note();
        var contract_id = $(this).parent().find('.co_id').text();
        var string = '/audit/index.php/Home/Index/list_editremark' + '?contract_id=' + contract_id;
        list_edit('备注', string, '', '300');
    });
    $(".remarks_extra").click(function() {
        note();
        var contract_id = $(this).parent().parent().find('.co_id').text();
        var string = '/audit/index.php/Home/Index/list_editextra' + '?contract_id=' + contract_id;
    ...
    $(".btn_end").click(function() {
        ...
        $.ajax({
            type: "POST",
            url: "/audit/index.php/Home/Index/list_end",
